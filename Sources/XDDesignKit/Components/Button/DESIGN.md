...
- `outline`：项目高频白底描边按钮，固定白色背景、`#222222` 前景和 `#CDCFD4` 固定 `1 pt` 边框。
- `outlineTransparent`：透明底描边按钮，固定 `#222222` 前景和 `#CDCFD4` 的固定 `1 pt` 边框；按下时使用浅灰反馈。
...
- `outline` 正常态和按下态边框颜色由 `#222222` 调整为固定 `#CDCFD4`；标题、背景、禁用态及 `outlineTransparent` 保持不变。
